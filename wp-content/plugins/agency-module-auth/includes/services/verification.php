...
function agency_auth_send_verification( $user_id ) {
	...
	if ( ! $user ) {
		return new WP_Error( 'agency_auth_user_missing', __( 'User not found.', 'agency-module-auth' ) );
	}
...
function agency_auth_verify_request() {
	...
	wp_die( esc_html__( 'Ez a megerősítő link érvénytelen vagy lejárt.', 'agency-module-auth' ), 403 );
}
